...
def store_digits(n):
    ...
    other_digits = int(temp[1:])
    return Link(first_digit, store_digits(other_digits))
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        validity = is_bst(b1) and is_bst(b2)
        return validity and bst_max(b1) <= t.label and bst_min(b2) > t.label
...
class Coin:
    ...
    def worth(self):
        ...
        return type(self).cents + extra if extra > 0 else type(self).cents
