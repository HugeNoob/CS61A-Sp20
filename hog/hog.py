...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    index, result, powered_score = 0, 0, score**3
    while powered_score != 0:
        if index%2 == 0:
            result += powered_score%10
        else:
            result -= powered_score%10
        index += 1
        powered_score = powered_score//10
    return 1 + abs(result)
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    value = 3 ** (player_score + opponent_score)
    return str(value)[0] == str(value%10)
    # END PROBLEM 4
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0, score1):
        if who:
            turn_gain = score1 - prev_score
            if turn_gain > prev_high:
                print("{} point(s)! That's the biggest gain yet for Player 1".format(turn_gain))
                return announce_highest(who, turn_gain, score1)
            return announce_highest(who, prev_high, score1)

        else:
            turn_gain = score0 - prev_score
            if turn_gain > prev_high:
                print("{} point(s)! That's the biggest gain yet for Player 0".format(turn_gain))
                return announce_highest(who, turn_gain, score0)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if is_swap(zero_roll_score, opponent_score):
        if opponent_score >= zero_roll_score:
            return 0
        return num_rolls
    if zero_roll_score - score >= margin:
            return 0
    return num_rolls
